...
public class Elevator {
    ...
    private int currLevel; // offSet by 1
    ...
    public void handleExternalRequest(ExternalRequest r)
    {
        ...
        else // r.getDirection() == Direction.DOWN
        {
    ...
    public void handleInternalRequest(InternalRequest r) // record the status????????
    {
        // check valid
        if(status == Status.UP)
        {
    ...
    public void closeGate()
    {
        ...
        else if(status == Status.UP)
        {
            ...
            {
                if(noRequests(downStops))
                { // no more downStops requests, and no upStops requests --> set to idle
                    status = Status.IDLE;
                } // downStops requests there is, change status to DOWN as it can go down now
                else status = Status.DOWN; // within the loop, change direction
            }
        }
        else {
            if(noRequests(downStops))
